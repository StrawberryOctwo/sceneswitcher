...
def crop_to_aspect_ratio(
    video: VideoFileClip, desired_aspect_ratio: float
) -> VideoFileClip:
...
def convert_color(color):
    ...
    if isinstance(color, str):
        ...
        raise ValueError(
            "Color format not recognized. Provide a hex string, named color, or RGB tuple as a string."
        )
...
def add_subtitles_to_clip(
    ...
    padding = 10
    subtitle_duration = subriptime_to_seconds(subtitle.end) - subriptime_to_seconds(
        subtitle.start
    )
    ...
    if text_width + padding <= max_box_width:
        ...
        subtitle_clip = TextClip(
            ...
            size=(
                max_box_width,
                None,
            ),  # Width constrained by max_box_width, height auto
        ).set_duration(subtitle_duration)
        ...
        max_real_width = max(
            [
                TextClip(line, fontsize=font_size, font=font_path).size[0]
    ...
    subtitle_position = (
        "center",
        clip.h - box_height - margin + (box_height - text_height) / 2,
    )
...
def replace_video_segments(
    ...
    for replace_index, replacement_video in replacement_videos.items():
        if 0 <= replace_index < len(combined_segments):
            ...
            ):  # if the subtitle length is longer than the replacement video
                subtitle_length = replacement_video.duration

            replacement_segment = replacement_video.subclip(
                0, subtitle_length
            )  # always crop to subtitle length, from 0
            replacement_segment = adjust_segment_duration(
                replacement_segment, target_duration
            )
...
def generate_srt_from_txt_and_audio(
    txt_file: Path, audio_file: Path, output_folder: Path
) -> Path:
...
def refine_subtitles_based_on_computer_vision(
    subtitles: pysrt.SubRipFile, timestamps: List[Dict], replacements: List[Dict]
) -> pysrt.SubRipFile:
    candidate_timestamps = []
    logging.debug(f"Refining {len(subtitles)} subtitles")
    for ts in timestamps:
        if ts["confidence"] > MAE_THRESHOLD:
            if len(candidate_timestamps) == 0 or (
                ts["timestamp"] - candidate_timestamps[-1]["timestamp"]
                > GLITCH_IGNORE_THRESHOLD
            ):
    ...
    logging.info(
        f"Found {len(candidate_timestamps)} candidate timestamps for subtitle changes"
    )
    if len(candidate_timestamps) != len(subtitles) - 1:
        logging.warning(
            f"The number of candidate timestamps does not match the number of subtitles, {len(subtitles)} vs {len(candidate_timestamps)}"
        )
    ...
    for subtitle in subtitles:
        if last_subtitle_record is not None:
            if (
                last_subtitle_record.end > subtitle.start
                or last_subtitle_record.end < subtitle.start
            ):
                logging.warning(
                    f"Subtitle overlap detected: [{last_subtitle_record.text}] - [{subtitle.text}]"
                )
        ...
        while cursor < len(candidate_timestamps):
            ...
            if candidate["timestamp"] < subtitle_end - 0.25:
                cursor += 1
                logging.debug(
                    f"Skipping candidate timestamp {candidate['timestamp']} for subtitle [{subtitle.text}]"
                )
                continue
            if candidate["timestamp"] > subtitle_end + 1.5:
                ...
                break
            logging.debug(
                f"Found candidate timestamp {candidate['timestamp']} for subtitle [{subtitle.text}]"
            )
    ...
    for i, subtitle in enumerate(subtitles):
        if i > 0:
            if i in clips and i - 1 not in clips:
                logging.debug(
                    f"Subtitle #{i} is a clip start, starting at: {subriptime_to_seconds(subtitle.start)}, line: {subtitle.text}"
                )
...
def main(
    video_clips_path,
    my_video,
    mp3_file_of_same_video,
    txt_file_of_same_video,
    output_folder,
    font_path,
    font_size,
    font_color,
    ...
    srt_file = generate_srt_from_txt_and_audio(
        Path(txt_file_of_same_video), Path(mp3_file_of_same_video), output_folder
    )
    ...
    refined_subtitles = refine_subtitles_based_on_computer_vision(
        subtitles, timestamps, replacement_base_folder
    )
    ...
    for folder in replacement_base_folder.iterdir():
        ...
        if not folder_name.isdigit():
            logging.warning(
                f"Folder name {folder_name} is not a valid segment index. Skipping..."
            )
            continue
        ...
        logging.info(
            f"Found {len(replacement_video_files)} replacement video files in {folder}"
        )
        ...
        for replacement_video_file in replacement_video_files:
            ...
            replacement_videos_per_combination[
                replacement_video_files.index(replacement_video_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_clips", "-ic", required=True, help="Input clips directory"
    )
    ...
    parser.add_argument(
        "--font_file",
        "-fn",
        default="Montserrat-SemiBold.ttf",
        help="Font path for subtitles",
    )
    parser.add_argument(
        "--font_size", "-fs", default=33, type=int, help="Font size for subtitles"
    )
    parser.add_argument(
        "--font_color", "-fc", default="white", help="Font color for subtitles"
    )
    parser.add_argument(
        "--bg_color", "-bc", default="black", help="Background color for subtitles"
    )
    parser.add_argument(
        "--margin", "-m", default=20, type=int, help="Margin for subtitles"
    )
